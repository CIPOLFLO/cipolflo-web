...
describe('CostoCuotaCard', () => {
  ...
  it('precarga el monto vigente al iniciar', () => {
    expect(component['form'].value.monto).toBe('100');
  });
  ...
  it('marca error cuando el monto es 0 o negativo', () => {
    component['onValuesChange']({ monto: '0' });
    expect(component['errors']()['monto']).toBe('El costo debe ser mayor a 0.');
  });

  it('marca error cuando el monto está vacío y se envía', () => {
    component['onValuesChange']({ monto: '' });
    component['onGuardar']();
    expect(component['errors']()['monto']).toBe('El costo de cuota es obligatorio.');
  });

  it('onGuardar no llama al service si el form es inválido', () => {
    component['onValuesChange']({ monto: '-5' });
    component['onGuardar']();
    expect(mockService.actualizar).not.toHaveBeenCalled();
  });

  it('onGuardar llama a actualizar con el monto ingresado', () => {
    component['onValuesChange']({ monto: '250' });
    component['onGuardar']();
  ...
  it('oculta la confirmación de guardado al volver a editar', () => {
    component['onValuesChange']({ monto: '250' });
    component['onGuardar']();
    component['onValuesChange']({ monto: '260' });
